...
As of Dioxus 0.7.3, server functions support **server-only extractors**. These are types that can only be extracted on the server side and are not sent from the client. This is useful for accessing server-specific state like databases, request headers, or session information:

```rust
#[get("/api/admin/stats")]
async fn get_admin_stats(db: SqlDb, headers: HeaderMap) -> Result<Stats> {
    // `db` and `headers` are only available on the server
    // They are not serialized from the client
    db.get_stats().await
}
```

Server-only extractors are automatically detected and skipped when generating the client-side request. You can mix server-only extractors with regular arguments freely.

### Query String Support

As of Dioxus 0.7.1, server functions use `serde_qs` for query string serialization. This enables complex nested query parameters that are not possible with simple `key=value` pairs:
...
async fn search(filter: Filter) -> Result<Vec<Product>> {
    // `filter` is deserialized from the query string using serde_qs
