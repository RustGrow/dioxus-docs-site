...
Dioxus supports **CSS Modules** and **Scoped CSS** out of the box, allowing you to write component-scoped styles without worrying about global class name collisions.

### CSS Modules

CSS Modules are CSS files where all class names are locally scoped by default. To use CSS Modules, name your file with the `.module.css` extension and import it with the `asset!()` macro:
...
fn App() -> Element {
    rsx! {
        ...
        div {
            class: "container",
            h1 { class: "title", "Hello from CSS Modules!" }
        }
    }
}
```

Under the hood, Dioxus transforms the class names into unique identifiers, preventing collisions between different components.

### Scoped CSS

For quick component-scoped styles, you can use the `style` element with the `scoped` attribute (or use inline style blocks). Scoped CSS ensures that the styles only apply to the component where they are defined:
...
> **Note:** Scoped CSS support may vary by renderer. CSS Modules are the recommended approach for production applications.

## Related Examples

- [Dynamic Classes](https://github.com/DioxusLabs/dioxus/tree/main/examples/02-building-ui/dynamic_classes.rs) — Changing CSS classes at runtime
- [Inline Styles](https://github.com/DioxusLabs/dioxus/tree/main/examples/03-assets-styling/inline_styles.rs) — Style attributes in RSX
- [Dynamic Styles](https://github.com/DioxusLabs/dioxus/tree/main/examples/03-assets-styling/dynamic_styles.rs) — Updating CSS dynamically
- [CSS Modules](https://github.com/DioxusLabs/dioxus/tree/main/examples/03-assets-styling/css_modules.rs) — Scoped CSS with modules
- [Stylesheet](https://github.com/DioxusLabs/dioxus/tree/main/examples/03-assets-styling/stylesheet.rs) — Loading CSS files
- [Tailwind Integration](https://github.com/DioxusLabs/dioxus/tree/main/examples/10-integrations/tailwind) — Full TailwindCSS setup
