...
The order the hooks are run must be the same because the order determines which hook gets what state! If you run the hooks in a different order, Dioxus may panic because it can't turn the state back into the right type, or you may just get the wrong state for your hook.
...
Dioxus provides many built-in hooks:

- `use_signal` — creates a reactive signal
- `use_memo` — creates a memoized computation
- `use_effect` — runs a side effect
- `use_resource` — manages asynchronous data
- `use_context` — reads context from the component tree
- `use_context_provider` — provides context to child components
