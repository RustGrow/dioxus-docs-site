...
## Comparing State Sharing Methods

Dioxus provides three different ways to pass around state between components. Context is just one of them:

1. **Pass values as props** — the most explicit and local approach. Use this when it isn't overly annoying to pass a value through a few layers.

2. **Use context** — great for state that is global to a subtree of your app. It lets you create multiple "global-ish" states while still making state different when you reuse components.

3. **Use global signals** — very ergonomic if your state is truly global, but you shouldn't use it if you need state to be different for different instances of your component. Libraries should generally avoid globals to make components more reusable.

The rest of this page focuses on context. For global signals, see the [Global Signals](#global-signals) section at the bottom of this page.
...
- [Context API](https://github.com/DioxusLabs/dioxus/tree/main/examples/04-managing-state/context_api.rs) — Sharing state via context
- [Global](https://github.com/DioxusLabs/dioxus/tree/main/examples/04-managing-state/global.rs) — Global state with signals
- [Lifting State](https://github.com/DioxusLabs/dioxus/tree/main/examples/04-managing-state/lifting_state.rs) — Moving state up the component tree
- [TodoMVC Store](https://github.com/DioxusLabs/dioxus/tree/main/examples/01-app-demos/todomvc_store.rs) — TodoMVC with a global store pattern
