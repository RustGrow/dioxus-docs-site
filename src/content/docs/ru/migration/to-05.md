---
title: Миграция на 0.5
---

# Как обновиться до Dioxus 0.5

В этом руководстве описаны изменения API между релизами `0.4` и `0.5`.

`0.5` включает значительные изменения в хуках, пропсах и глобальном состоянии.

## Шпаргалка

Вот краткая шпаргалка по изменениям:
...
Подробнее о новом API пропсов читайте в руководстве по [миграции пропсов](to-05-props.md).
...
use_future((dependency1, dependency2,), move |(dependency1, dependency2,)| async move {
	/*используем dependency1 и dependency2*/
});
```
Dioxus 0.5:
```rust
...
Подробнее о хуке `use_resource` читайте в руководстве по [миграции хуков](to-05-hooks.md).
...
Подробнее о хуке `use_signal` читайте в руководстве по [миграции состояния](to-05-state.md).
