...
You can define catch-all routes using the `#[route("/:..segments")]` attribute:
...
To navigate between routes, you can use the `Link` component or the `use_navigator` hook:
...
You can nest routes to create page hierarchies:
...
Now our HotDog app has multiple pages with type-safe navigation! Users can browse dogs, save them to favorites, and view their collection.
