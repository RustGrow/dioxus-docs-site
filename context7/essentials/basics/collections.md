...
### Granular Subscriptions with Nested Stores

One of the key advantages of stores is that they lazily create signals for each field and collection element. This means modifying one field or pushing to a vector will only re-render the parts of your app that actually depend on that specific piece of data.

Here is a more complex example that demonstrates stores with a recursive tree structure:
...
fn Tree(value: Store<CounterTree>) -> Element {
    // Calling the generated `count` method returns a new store scoped to just the count field
    ...
    rsx! {
        ...
        button {
            // Stores are aware of data structures like Vec and HashMap. When we push an item,
...
In this example, clicking "Increment" only re-renders the count display for that specific tree node, not its children or siblings. Clicking "Push child" only updates the list of children without re-rendering existing child components. This granular reactivity is especially powerful for large nested data structures.
