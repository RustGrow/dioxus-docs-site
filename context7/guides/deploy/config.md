...
## Windows App Icon Bundling

As of Dioxus 0.7.6, `dx serve` and `dx bundle` support bundling Windows app icons. To configure the icon path, set the `icon_path` field in the `[bundle.windows]` section of your `Dioxus.toml`:
...
The icon will be embedded into the executable and displayed in the taskbar and window title bar.
