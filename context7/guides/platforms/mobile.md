...
## Customizing Mobile Builds

As of Dioxus 0.7.4, you can customize all aspects of iOS and Android builds via `Dioxus.toml`. The CLI schema includes fields for:

- **Android**: SDK version, NDK path, signing configuration, permissions, and manifest customization
- **iOS**: Bundle identifier, team ID, signing configuration, device targeting, and LaunchScreen storyboard support

For the full list of available fields, refer to the [CLI schema](https://github.com/DioxusLabs/dioxus/blob/main/packages/cli/schema.json).

## FFI for Mobile

As of Dioxus 0.7.4, Dioxus includes a new FFI interface for **Kotlin**, **Java**, and **Swift**. This allows you to call platform-native APIs directly from your Rust code. The FFI system automatically bundles relevant source files into your mobile builds.

To use the FFI interface, place your platform-specific code in the appropriate directories and reference them in your `Dioxus.toml`:
...
TypeScript/JavaScript FFI support is planned for a future release.
